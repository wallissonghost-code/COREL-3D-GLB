...
function init(){
 ...
 </select><small>muda como a malha reage ao pincel</small></label>
 ...
 <button id="sculptSubdivide" class="primary">➕ Subdividir peça para mais detalhes</button>
 <small id="sculptInfo" class="sub">Passe o mouse sobre uma peça para ver a densidade da malha.</small>
 <small class="sub">Elevar cria volume • Afundar cava • Inflar engrossa • Achatar nivela • Pinçar concentra vértices • Crease cria sulco • Suavizar remove irregularidades. Botão direito continua girando a câmera.</small>`;
 ...
 function stats(mesh){if(!mesh?.geometry)return;const g=mesh.geometry,p=g.attributes.position;const tri=Math.round((g.index?g.index.count:p.count)/3);$('sculptInfo').textContent=`Malha sob o cursor: ${p.count.toLocaleString('pt-BR')} vértices • ${tri.toLocaleString('pt-BR')} triângulos`;}
 ...
 function normalLocal(mesh,hit){const n=hit.face?.normal?.clone()||new THREE.Vector3(0,0,1);return n.normalize();}
 function deform(hit){
  const mesh=hit.object;if(!mesh||!mesh.geometry?.attributes?.position)return;
  ...
  const local=mesh.worldToLocal(hit.point.clone()),rad=radius(),str=strength(),mode=$('sculptMode').value,planeN=normalLocal(mesh,hit);
  const affected=[];let cx=0,cy=0,cz=0,ws=0;
  for(let i=0;i<pos.count;i++){
   const x=pos.getX(i),y=pos.getY(i),z=pos.getZ(i),dx=x-local.x,dy=y-local.y,dz=z-local.z,d=Math.hypot(dx,dy,dz);
   if(d<=rad){const fall=Math.pow(1-d/rad,2);affected.push([i,fall,x,y,z]);cx+=x*fall;cy+=y*fall;cz+=z*fall;ws+=fall;}
  }
  if(!affected.length)return;cx/=ws||1;cy/=ws||1;cz/=ws||1;
  for(const [i,fall,x,y,z] of affected){
   const k=str*fall,dx=x-local.x,dy=y-local.y,dz=z-local.z;
   let nx=nor?nor.getX(i):0,ny=nor?nor.getY(i):0,nz=nor?nor.getZ(i):1;const nl=Math.hypot(nx,ny,nz)||1;nx/=nl;ny/=nl;nz/=nl;
   if(mode==='raise'||mode==='lower'){
    const sign=mode==='lower'?-1:1,amt=sign*k*.55;pos.setXYZ(i,x+nx*amt,y+ny*amt,z+nz*amt);
   }else if(mode==='inflate'){
    const amt=k*.75;pos.setXYZ(i,x+nx*amt,y+ny*amt,z+nz*amt);
   }else if(mode==='smooth'){
    const q=Math.min(.42,str*.14)*fall;pos.setXYZ(i,x+(cx-x)*q,y+(cy-y)*q,z+(cz-z)*q);
   }else if(mode==='flatten'){
    const dist=dx*planeN.x+dy*planeN.y+dz*planeN.z,q=Math.min(.65,k*.35);pos.setXYZ(i,x-planeN.x*dist*q,y-planeN.y*dist*q,z-planeN.z*dist*q);
   }else if(mode==='pinch'){
    const q=Math.min(.7,k*.22),tx=local.x-x,ty=local.y-y,tz=local.z-z,dot=tx*planeN.x+ty*planeN.y+tz*planeN.z;
    pos.setXYZ(i,x+(tx-planeN.x*dot)*q,y+(ty-planeN.y*dot)*q,z+(tz-planeN.z*dot)*q);
   }else if(mode==='crease'){
    const q=Math.min(.65,k*.18),tx=local.x-x,ty=local.y-y,tz=local.z-z,dot=tx*planeN.x+ty*planeN.y+tz*planeN.z,cut=k*.32;
    pos.setXYZ(i,x+(tx-planeN.x*dot)*q-nx*cut,y+(ty-planeN.y*dot)*q-ny*cut,z+(tz-planeN.z*dot)*q-nz*cut);
   }
  }
 ...
 function subdivide(mesh){
  if(!mesh?.geometry?.attributes?.position)return status('Passe o mouse sobre uma peça primeiro',true);
  const src=mesh.geometry.index?mesh.geometry.toNonIndexed():mesh.geometry.clone(),p=src.attributes.position,tri=p.count/3;
  if(tri>60000){src.dispose();return status('Malha já está muito pesada para subdividir com segurança',true)}
  const out=[];
  const v=i=>new THREE.Vector3(p.getX(i),p.getY(i),p.getZ(i));
  const push=(a,b,c)=>out.push(a.x,a.y,a.z,b.x,b.y,b.z,c.x,c.y,c.z);
  for(let i=0;i<p.count;i+=3){const a=v(i),b=v(i+1),c=v(i+2),ab=a.clone().add(b).multiplyScalar(.5),bc=b.clone().add(c).multiplyScalar(.5),ca=c.clone().add(a).multiplyScalar(.5);push(a,ab,ca);push(ab,b,bc);push(ca,bc,c);push(ab,bc,ca);}
  const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(out,3));g.computeVertexNormals();g.computeBoundingBox();g.computeBoundingSphere();
  undo.push({mesh,arr:new Float32Array(mesh.geometry.attributes.position.array),geometry:mesh.geometry.clone()});if(undo.length>40)undo.shift();
  const old=mesh.geometry;mesh.geometry=g;old.dispose();src.dispose();stats(mesh);status(`Malha subdividida: ${Math.round(out.length/9).toLocaleString('pt-BR')} triângulos`);
 }
 $('sculptToggle').onclick=()=>{active=!active;$('sculptToggle').textContent=active?'✅ Pincel ativo':'🖌️ Ativar pincel';$('sculptToggle').classList.toggle('primary',active);cursor.visible=false;status(active?'Pincel 3D ativo • arraste sobre a peça':'Pincel 3D desativado')};
 ...
 $('sculptReset').onclick=()=>{if(!undo.length)return status('Nenhum detalhe para limpar',true);const first=undo[0],mesh=first.mesh;if(first.geometry){const old=mesh.geometry;mesh.geometry=first.geometry.clone();old.dispose();}else if(mesh?.geometry?.attributes?.position&&mesh.geometry.attributes.position.array.length===first.arr.length){mesh.geometry.attributes.position.array.set(first.arr);mesh.geometry.attributes.position.needsUpdate=true;mesh.geometry.computeVertexNormals();}undo=[];status('Detalhes do pincel limpos')};
 ...
 canvas.addEventListener('pointerdown',e=>{if(!active||e.button!==0)return;const h=hitAt(e);if(!h)return;e.preventDefault();painting=true;currentMesh=h.object;lastHovered=h.object;saveStroke(currentMesh);deform(h);},true);
