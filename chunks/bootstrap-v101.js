...
try {
  const loaded = await Promise.all(files.map(async name => {
    const url = new URL(`${name}?v=124`, import.meta.url);
    const response = await fetch(url, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Falha ao carregar ${url.pathname}: HTTP ${response.status}`);
    return `\n/* ===== ${name} ===== */\n${await response.text()}`;
  }));

  // Os chunks v101 são fragmentos de um único programa e alguns terminam no
  // meio de funções que continuam no arquivo seguinte. Portanto a sintaxe só
  // pode ser validada depois da concatenação completa.
